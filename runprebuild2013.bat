@echo off
::
:: Prebuild generator for the METAbolt
::
...
if(.%1)==(.) echo %SystemRoot%\Microsoft.NET\Framework\v3.5\msbuild METAbolt.sln >> compile.bat

if(.%1)==(.msbuild) echo echo ==== COMPILE BEGIN ==== >> compile.bat
if(.%1)==(.msbuild) echo %SystemRoot%\Microsoft.NET\Framework\v3.5\MSBuild.exe /p:Configuration=Release METAbolt.sln >> compile.bat
...
if(.%2)==(.docs) echo %SystemRoot%\Microsoft.NET\Framework\v3.5\MSBuild.exe /p:Configuration=Release docs\METAbolt.shfbproj >> compile.bat
...
if(.%2)==(.runtests) echo nunit-console bin\METAbolt.Tests.dll /exclude:Network /nodots /labels /xml:testresults.xml >> compile.bat
...
copy METAbolt\METAbolt.nsi bin
echo del /q METAbolt-*-installer.exe >> compile.bat
echo del /q METAbolt-latest.zip >> compile.bat

if not exist "%PROGRAMFILES%\NSIS\Unicode\makensis.exe" goto NOUNINSIS
echo "%PROGRAMFILES%\NSIS\Unicode\makensis.exe" bin\METAbolt.nsi >> compile.bat
goto NONSIS
:NOUNINSIS

if not exist "%PROGRAMFILES%\NSIS\makensis.exe" goto NONSIS
echo "%PROGRAMFILES%\NSIS\makensis.exe" bin\METAbolt.nsi >> compile.bat
:NONSIS

echo cd bin >> compile.bat
echo 7z.exe a -r -tzip ..\METAbolt-latest.zip *.* >> compile.bat
echo cd .. >> compile.bat
